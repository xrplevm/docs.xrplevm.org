...
2. Use the Axelar network's GMP functionality to execute a smart contract directly from  the XRPL. See: [Relay messages manually](../evm-sidechain/axelar-manually-relay-messages.md).


## Liquidity Access

The XRP Ledger is now integrated with the Axelar network, giving it access to the functionality and liquidity of any chain connected to Axelar. Moving XRP and tokens to and from the XRPL has never been easier. To learn how quickly assets can be bridged, see: [Axelar Interchain Transfer](../evm-sidechain/axelar-interchain-transfer.md).